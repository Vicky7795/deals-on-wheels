  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setProfileMsg({ error: '', success: 'Seller profile updated successfully!' });
      }
    } catch (err) {
      setProfileMsg({ error: err.response?.data?.message || 'Failed to update seller profile.', success: '' });
    } finally {
      ...
      <main className="flex-1 p-6 lg:p-8 space-y-8 max-w-4xl mx-auto">
        <div className="border-b border-gray-200 pb-4">
          <h1 className="text-2xl font-extrabold text-gray-900 flex items-center gap-2">
            <User className="w-6 h-6 text-blue-600" /> Seller Profile & Settings
          </h1>
          <p className="text-sm text-gray-500 mt-1">Manage your seller contact details and account credentials</p>
        </div>

        {/* Profile Details Form */}
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6">
          <h3 className="text-lg font-bold text-gray-900 border-b border-gray-100 pb-3">Seller Business Info</h3>
          ...
          <form onSubmit={handleProfileSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Full Name / Dealership Name</label>
                <input
                  type="text"
                  required
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Contact Phone Number</label>
                <input
                  type="text"
                  required
